    ...
    <table class="table-responsive table-condensed table-striped table-hover table-bordered">
        <thead>
            ...
            <tr>
                ...
                <td class="center bold" style="font-size:20pt" colspan="{{ count($nosj) + 5 }}">
                    REKAP DELIVERY & TAGIHAN ED-COATING
                </td>
            </tr>
            <tr>
                <td class="center bold" style="font-size: 18pt;" colspan="{{ count($nosj) + 5 }}">
                    {{ $dat[0]->customer['name'] }}
                </td>
            </tr>
            <tr>
                <td colspan="2" class="bold border-right">PERIODE</td>
                <td colspan="{{ count($nosj) + 5 }}" class="bold">: <?php echo strtoupper(tanggal_indonesia($datdetail[0]['uniqe'][0]['date_sj'], 'periode')); ?></td>
            </tr>
            <tr>
                <td colspan="2" class="bold border-right">NO PO</td>

                <td colspan="{{ count($nosj) + 5 }}" class="bold">: <?php if ($po) {
                                                                        echo $po->no_po;
                                                                    } ?></td>
            </tr>
            <tr>
                <td colspan="2" class="bold border-right">JASA PROSES</td>
                <td colspan="{{ count($nosj) + 5 }}" class="bold">: ED-COATING</td>
            </tr>
            <tr class="bold headercolor" style="background-color: #DDEBF7">
                ...
                <td class="center" colspan="{{ count($nosj) }}">SURAT JALAN</td>
            ...
            <tr class="headercolor" style="background-color: #DDEBF7">
                <?php foreach (collect($nosj)->sortBy('nosj') as $att) { ?>
        ...
        <tbody>
            ...
            for ($c = 0; $c < count($datdetail); $c++) { ?>
                <tr>
                    ...
                    <?php for ($f = 0; $f < count($nosj); $f++) {
                        if (isset($nosj[$f])) {
                            foreach (collect($datdetail[$c]['uniqe'])->sortBy('nosj_link') as $sj) {
                                if (isset($sj['sj_real']) && $nosj[$f]['nosj'] === $sj['sj_real'][0]['nosj']) { ?>
                                    <td class="center qtysum">{{ $sj['sj_real'][0]['qty'] }}</td>
                                <?php } else { ?>
                                    <td class="center qtysum">1</td>
            ...
            <tr>
                <td class="bold" style="text-align: right;padding-right:10px" colspan="{{ count($nosj) + 5 }}">TOTAL</td>
                <td class="bold">
                    <nav class="container">
                        <div>Rp</div>
                        <div id="total"></div>
            ...
            <tr>
                <td class="bold" style="text-align: right;padding-right:10px" colspan="{{ count($nosj) + 5 }}">PPN {{$pajak->ppn}}%</td>
                <td class="bold">
                    <nav class="container">
                        <div>Rp</div>
                        <div id="ppn"></div>
            ...
            <tr>
                <td class="bold" style="text-align: right;padding-right:10px" colspan="{{ count($nosj) + 5 }}">PPH 23 {{$pajak->pph}}%</td>
                <td class="bold">
                    <nav class="container">
                        <div>Rp</div>
                        <div id="pph"></div>
            ...
            <tr>
                <td class="bold" style="text-align: right;padding-right:10px" colspan="{{ count($nosj) + 5 }}">Grand Total</td>
                <td class="bold">
                    <nav class="container">
                        <div>Rp</div>
